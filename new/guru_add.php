  ...
  <form method="post" enctype="multipart/form-data">
    ...
    <?php
      ...
      if(isset($_POST['submit'])){
        ...
        // Create a validation
        if(empty($nik) || empty($name) || empty($kelas) || empty($jurusan)){
          echo '
        ...
        } else if(count((array) $connect->query("SELECT nik FROM student WHERE nik = '$nik'")->fetch_array()) > 1) {
          echo '
            <div class="bg-warning rounded-2 my-3 pt-2 pb-2">
              <p class="p text-center my-auto">
          ...
          // if the file format is not meet with the requirements from $format
            if($filesize < 10044070) {
              ...
              $connect->query("INSERT INTO student(nik, nama, kelas, jurusan, photo) VALUES ('$nik', '$name', '$kelas', '$jurusan', '$_photo')");
              echo '
                <script>
                  Swal.fire({
                    icon: \'success\',
      ...
      <tr>
        ...
        <td>
          <select name="kelas" id="i-kelas">
            ...
            <option value="10">SMK Kelas 10</option>
            <option value="11">SMK Kelas 11</option>
            <option value="12">SMK Kelas 12</option>
          </select>
        </td>
      </tr>
      <tr>
